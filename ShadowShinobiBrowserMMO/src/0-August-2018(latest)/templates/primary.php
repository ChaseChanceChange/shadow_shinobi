...
(function(){
  ...
  window.mostrartreino = function(numero,recompensa,requerimento,localtreino){ var el=document.getElementById('elemento'+numero); if(!el)return; el.innerHTML='<div class="ss-detail-grid"><div><span>Requirement</span>'+requerimento+'</div><div><span>Reward</span>'+recompensa+'</div><div><span>Discipline Site</span>'+localtreino+'</div></div>'; };
  window.mostrarquest = function(numero,recompensa,requerimento,localtreino,nivel,conclusao,itemprocurado,local,monstroprocurado){ var el=document.getElementById('elemento'+numero); if(!el)return; var links=''; if(local)links+=' <button type="button" class="ss-inline-link" onclick="openmappopup(\\''+String(local).replace(/\\'/g,'')+'\\')">Map</button>'; if(monstroprocurado)links+=' <button type="button" class="ss-inline-link" onclick="openmapmonster(\\''+String(monstroprocurado).replace(/\\'/g,'')+'\\')">Threat</button>'; if(itemprocurado)links+=' <button type="button" class="ss-inline-link" onclick="openmaprespawn(\\''+String(itemprocurado).replace(/\\'/g,'')+'\\')">Recovery</button>'; el.innerHTML='<div class="ss-detail-grid"><div><span>Requirement</span>'+requerimento+links+'</div><div><span>Reward</span>'+recompensa+'</div><div><span>Completion</span>'+conclusao+'</div><div><span>Standing</span>'+nivel+'</div><div><span>Location</span>'+localtreino+'</div></div>'; };
  window.mostrargraduacao = function(nomeelemento,requerimento,ganhos,itemprocurado){ var el=document.getElementById(nomeelemento); if(!el)return; el.innerHTML='<div class="ss-detail-grid"><div><span>Requirement</span>'+requerimento+'</div><div><span>Bonus</span>'+ganhos+'</div>'+(itemprocurado?'<div><span>Required gear</span>'+itemprocurado+'</div>':'')+'</div>'; };
  ...
  window.procurarjogador = function(){ var el=getNode('procurarjog'); if(!el)return; el.innerHTML='<form class="ss-search-inline" onsubmit="event.preventDefault();mostrarchar(this.nome.value)"><input id="ss-player-search" name="nome" maxlength="30" placeholder="Operative name"><button type="submit">View</button></form>'; };
