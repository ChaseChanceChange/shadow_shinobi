...
$qual = $_GET['qual'];
    ...
    /* testando se est? logado */
		...
		// $userrow = checkcookies();
		 global $userrow;
		if ($userrow == false) { display("Por favor fa̤a o <a href=\"login.php?do=login\">log in</a> no jogo antes de executar essa a̤̣o.","Erro",false,false,false);die(); }
		if ($userrow["currentaction"] != "In Town") {if ($userrow["currentaction"] == "Fighting"){header('Location: ./index.php?do=fight&conteudo=Voce so pode acessar essa funcao dentro de uma cidade!');die();}else{header('Location: ./index.php?conteudo=Voce so pode acessar essa funcao dentro de uma cidade!');die();} }
			if ($userrow["currentaction"] == "Fighting") {header('Location: ./index.php?do=fight&conteudo=Voce nao pode acessar essa funcao no meio de uma batalha!');die(); }
			if ($qual > 6) {display("Tentativa de trapa̤a ou erro detectado.","Erro",false,false,false);die(); }
			if ($qual < 1) {display("Tentativa de trapa̤a ou erro detectado.","Erro",false,false,false);die(); }
...
$qual = $_GET['qual'];
    ...
    /* testando se est? logado */
		...
		if ($userrow["currentaction"] != "In Town") {if ($userrow["currentaction"] == "Fighting"){header('Location: ./index.php?do=fight&conteudo=Voce so pode acessar essa funcao dentro de uma cidade!');die();}else{header('Location: ./index.php?conteudo=Voce so pode acessar essa funcao dentro de uma cidade!');die();} }
			...
			//fim
			if ($qual >= $quantidade) {display("Tentativa de trapa̤a ou erro detectado.","Erro",false,false,false);die(); }
	...
	//se nao tem item
	if ($itensnobanco[$qual] == ""){header('Location: http://nigeru.com/shadowshinobi/funcaoitens.php?do=banco');die(); }
...
$qual = $_GET['qual'];
    ...
    /* testando se est? logado */
        ...
        extract($_POST);
			...
			//fim
			if ($qual >= $quantidade) {display("Tentativa de trapa̤a ou erro detectado(a).","Erro",false,false,false);die(); }
	...
	if (mysqli_num_rows($userquery2) != 1) { header('Location: ./funcaoitens.php?do=banco&bi=Nao existe nenhum Jogador com esse Nome para Doar o Item.');die(); }
...
$qual = $_GET['qual'];
    ...
    /* testando se est? logado */
		...
		if ($userrow["currentaction"] != "In Town") {if ($userrow["currentaction"] == "Fighting"){header('Location: ./index.php?do=fight&conteudo=Voce so pode acessar essa funcao dentro de uma cidade!');die();}else{header('Location: ./index.php?conteudo=Voce so pode acessar essa funcao dentro de uma cidade!');die();} }
			if ($userrow["currentaction"] == "Fighting") {header('Location: ./index.php?do=fight&conteudo=Voce nao pode acessar essa funcao no meio de uma batalha!');die(); }
			if ($qual > 4) {display("Tentativa de trapa̤a ou erro detectado.","Erro",false,false,false);die(); }
			if ($qual < 1) {display("Tentativa de trapa̤a ou erro detectado.","Erro",false,false,false);die(); }
...
$qual = $_GET['qual'];
    ...
    /* testando se est? logado */
        ...
        if (mysqli_num_rows($userquery) != 1) {  header('Location: ./funcaoitens.php?do=banco&br=Nao existe nenhum jogador com esse Nome.');die(); }
